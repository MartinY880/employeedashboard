// ProConnect — Lender Logo Upload API
// POST: Upload a lender logo image, returns { url }
...
const LOGOS_DIR = join(UPLOADS_BASE, "lender-logos");
...
export async function POST(request: Request) {
  try {
    ...
    const url = `/api/lenders/upload-logo/${storageName}`;

    return NextResponse.json({ url }, { status: 201 });
  } catch (err) {
    console.error("[Lender Logo Upload] error:", err);
